...
public class SlimeController : MovingObject
{
    ...
    private void Flip() //슬라임 공격 에니메이션을 왼쪽 방향만 만들었기 때문에 scale을 -1로 바꿔서 오른쪽 공격모션을 취할수 있게 바꿔줌
    {
        Vector3 flip = transform.localScale;
        if (PlayerPos.x > this.transform.position.x) //플레이어가 슬라임보다 오른쪽에 잇을경우
            flip.x = -1f;
        else
            flip.x = 1f;
        this.transform.localScale = flip;

        healthBar.transform.localScale = flip; //flip할경우 체력바를 다시 flip시켜서 원상태로 돌림

        animator.SetTrigger("Attack"); //trigger기때문에 true, false값이 필요없이 실행가능
    ...
    private bool NearPlayer() //몬스터가 공격을하려면 플레이어가 근처에 있어야함, 그것을 확인할 함수
    {
        ...
        {//비교값에 조금 여유를 둬서 미세한 오차범위 허용
            if (Mathf.Abs(Mathf.Abs(PlayerPos.y) - Mathf.Abs(this.transform.position.y)) <= speed * walkCount * 0.5)
            {//x좌표로만 1칸 떨어지고, y좌표 같을경우
                return true; // true 반환해서 플레이어가 근처에 있다는 것을 알려줌
            }
        }
        ...
        {
            if (Mathf.Abs(Mathf.Abs(PlayerPos.x) - Mathf.Abs(this.transform.position.x)) <= speed * walkCount * 0.5)
            {//x좌표 같고, y좌표로만 1칸 떨어진 경우
                return true;
            }
        }
